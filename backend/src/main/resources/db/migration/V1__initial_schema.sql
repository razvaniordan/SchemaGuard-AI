...
CREATE TABLE countries (
    ...
    CONSTRAINT ck_countries_code CHECK (country_code ~ '^[A-Z]{2}$')
);
-- -----------------------------------------------------------------------------
CREATE TABLE card_types (
    card_type_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE mcc_codes (
    ...
    CONSTRAINT ck_mcc_codes_code CHECK (mcc_code ~ '^[0-9]{4}$')
);
-- -----------------------------------------------------------------------------
...
CREATE TABLE acquiring_partners (
    acquiring_partner_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE card_networks (
    card_network_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE transactions (
    ...
    CONSTRAINT fk_transactions_mcc FOREIGN KEY (mcc_code) REFERENCES mcc_codes (mcc_code),
...
-- Important note: Some fields are technically derivable from card_id or merchant_id, such as issuer_bank_id, card_network_id and acquiring_partner_id.
--
-- However, we will still store them in transactions because a transaction should preserve the exact situation at the time it happened. 
-- If a merchant changes acquirer later, old transactions should not change meaning.
-- -----------------------------------------------------------------------------
...
ON interchange_rules (
    (COALESCE(card_network_id, -1::bigint)),
    (COALESCE(mcc_code, 'ANY')),
    (COALESCE(card_type_id, -1::bigint)),
...
CREATE TABLE transaction_simulations (
    simulation_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
    transaction_id BIGINT NOT NULL,
...
CREATE TABLE transaction_optimization_results (
    optimization_result_id BIGINT GENERATED BY DEFAULT AS IDENTITY,
    transaction_id BIGINT NOT NULL,
